...
set.seed(121)
split <- caret::createDataPartition(train_clean$target, p=0.85, list=FALSE)
train_clean <- train_clean[split, ]
validation <- train_clean[ -split, ]
...
preds1[preds1 >= 0.5] <- "Above"
preds1[preds1 < 0.5] <- "Below"
preds1 = as.factor(preds1)

preds2[preds2 >= 0.5] <- "Above"
preds2[preds2 < 0.5] <- "Below"
preds2 = as.factor(preds2)

preds3[preds3 >= 0.5] <- "Above"
preds3[preds3 < 0.5] <- "Below"
...
getROC <- function(model) {
    ...
    p1 <- data.frame(pred = validation$target, prob = pred.prob1[[1]])
    p1 <- p1[order(p1$prob),]
...
```{r}
# null_model <- glm(target ~ 1, data = train_clean, family = 'binomial')
# models <- list(logMod1, logMod2, logMod3)
# Predictor <- models %>% 
#   lapply(function(x) str_c(unlist(row.names(summary(x)$coefficients)), collapse = ',')) %>%
#   unlist() %>% str_remove('\\(Intercept\\)\\,')
# McFaddens_R2 <- list(1-logLik(logMod1)/logLik(null_model),
#                      1-logLik(logMod2)/logLik(null_model),
#                      1-logLik(logMod3)/logLik(null_model)) %>%
#   unlist()
# 
# AIC <- models %>%
